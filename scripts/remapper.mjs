...
p('---');
p();
p('## 8. Ce qui a été décidé — 20 septembre 2026');
p();
p('**Option C : archive, et rien d’autre.** Les 77 images restent où elles sont, avec');
p('leurs URLs, leur manifeste et leur vérificateur. **Aucun remappage n’a lieu.** La');
p('bibliothèque des 54 semaines part de zéro.');
p();
p('| question | décision | effet |');
p('| --- | --- | --- |');
p('| Les 52 matières | **archive** | `images/MM-JJ/` continue de servir ; rien ne bouge |');
p('| Les 25 scènes | **archive** | elles restent des journées complètes de l’ancien modèle |');
p('| Le chapitre « Les amoureux » | **deux personnes** | les scènes, qui montrent une personne seule, ne peuvent pas y prétendre — c’est écrit dans `scripts/semaines.mjs` (`REGLES_DE_CHAPITRE`) et rappelé par `npm run semaines:lot` |');
p('| Les 365 dossiers de jour vides | **gardés** | ils restent pour mémoire ; leur suppression est une décision séparée |');
p('| Le format de la collection | **A4 portrait, 1240 × 1754** | ce n’est pas du 5 / 7 — le normaliseur et le vérificateur savent lequel s’applique à quel dossier |');
p();
p('**Ce que ça veut dire concrètement :** l’ancienne bibliothèque est un témoin, pas');
p('un gisement. Le nouveau modèle ne réutilise rien d’elle — et ne dépend donc de');
p('rien de ce qu’elle contient.');
p();
p('---');
p();
p('## 9. Ce que le remappage n’aurait pas fait');
p();
p('- **Il n’aurait pas renommé** `images/MM-JJ/` en `images/semaine-NN/` : ce sont');
p('  deux structures différentes, pas deux écritures du même nom.');
p('- **Il n’aurait pas touché** au manifeste des jours : il reste la déclaration de');
p('  ce qui a été produit sous l’ancien modèle.');
p('- **Il n’aurait pas franchi la frontière des saisons** : une matière d’hiver ne');
p('  devient pas une couverture d’août.');
p();
